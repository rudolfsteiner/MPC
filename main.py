...
def sample(env, 
           ...
           verbose=False):
    """
        sampler function which takes in an environment, a controller (either random or the MPC controller), 
        and returns rollouts by running on the env. 
        Each path can have elements for observations, next_observations, rewards, returns, actions, etc.
    """
    paths = []
    
    #print("sample:", type(controller))
    for i in tqdm.tqdm(range(num_paths)):
        obs_h, ac_h, obs_next_h, rew_h = [], [], [], []
        obs = env.reset()
        steps = 0
        ...
        path = {'observations': np.array(obs_h),
                'actions': np.array(ac_h),
                'next_observations': np.array(obs_next_h),
                'rewards': np.array(rew_h)}
